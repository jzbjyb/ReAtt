...
class ReAttConfig(T5Config):
    def __init__(
        self,
        *args,
        retrieval_corpus: str = None,
        retrieval_layer: int = 6,
        retrieval_head: int = 0,
...
class ReAttAttention(T5Attention):
    def __init__(
        self,
        config: ReAttConfig,
    ...
    def forward(
        ...
        if output_attentions:
            ...
            else:  # use the attention slot for retrieval-related stuff
                if self.training:  # return attention matrix for learning at training-time
                    outputs = outputs + (attn_weights,)
                else:
                    outputs = outputs + (
...
class ReAttLayerSelfAttention(nn.Module):
    def __init__(
        self,
        config: ReAttConfig,
        has_relative_attention_bias: bool = False,
        ...
        super().__init__()
        self.SelfAttention = ReAttAttention(config, has_relative_attention_bias=has_relative_attention_bias, layer_index=layer_index)
...
class ReAttBlock(nn.Module):
    def __init__(
        ...
        self.layer.append(ReAttLayerSelfAttention(
            config,
...
class ReAttPreTrainedModel(T5PreTrainedModel):
...
class ReAttStack(T5PreTrainedModel):
    ...
    def forward(
        ...
        for i, (layer_module, past_key_value) in enumerate(zip(self.block, past_key_values)):
            ...
            if only_biencoder and i >= self.retrieval_layer:
                # run layers until the retrieval_layer
                break
...
class ReAttForConditionalGeneration(ReAttPreTrainedModel):
    ...
    def __init__(self, config: ReAttConfig):
        super().__init__(config)
        ...
        self.decoder = ReAttStack(decoder_config, self.shared)
...
class ReAttRetriever:
    ...
    def __init__(self, encoder: ReAttStack):
        ...
        self.retrieval_corpus = config.retrieval_corpus
        self.vector_dimension = config.d_kv
        self.retrieval_layer = config.retrieval_layer
    ...
    def encode(
        ...
        attention_mask: torch.LongTensor  # (bs, seq_len)
